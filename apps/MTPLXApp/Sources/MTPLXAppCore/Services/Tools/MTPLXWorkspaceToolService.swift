...
struct MTPLXWorkspaceToolService: Sendable {
    // Repository configuration cannot turn a read into command execution.
    private static let readOnlyGit = "env GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null git --no-optional-locks -c core.fsmonitor=false -c core.hooksPath=/dev/null -c protocol.allow=never"
    ...
    private func inspectRepo(root: URL) async -> String {
        ...
        let result = await commandResult(
            "\(Self.readOnlyGit) branch --show-current",
            root: root,
            timeout: 10
        )
    ...
    private func gitStatus(root: URL) async -> String {
        let result = await commandResult("\(Self.readOnlyGit) status --short --branch --ignore-submodules=all", root: root, timeout: 10)
        return jsonObject([
            "command": "\(Self.readOnlyGit) status --short --branch --ignore-submodules=all",
    ...
    private func gitDiff(_ arguments: [String: Any], root: URL) async -> String {
        ...
        let commands: [(String, String)] = {
            switch scope {
            case "staged": return [("staged", "\(Self.readOnlyGit) diff --cached --no-ext-diff --no-textconv --ignore-submodules=all --unified=3\(pathSuffix)")]
            case "both": return [
                ("unstaged", "\(Self.readOnlyGit) diff --no-ext-diff --no-textconv --ignore-submodules=all --unified=3\(pathSuffix)"),
                ("staged", "\(Self.readOnlyGit) diff --cached --no-ext-diff --no-textconv --ignore-submodules=all --unified=3\(pathSuffix)")
            ]
            default: return [("unstaged", "\(Self.readOnlyGit) diff --no-ext-diff --no-textconv --ignore-submodules=all --unified=3\(pathSuffix)")]
            }
        }()
    ...
    private static func applyPatchSynchronously(_ patch: String, root: URL) -> PatchResult {
        ...
        do {
            ...
            let check = executeCommand(
                "\(Self.readOnlyGit) apply --check --whitespace=nowarn -- \(quotedPath)",
                root: root,
                timeout: 60
            )
            ...
            let applied = executeCommand(
                "\(Self.readOnlyGit) apply --whitespace=nowarn -- \(quotedPath)",
                root: root,
                timeout: 60
            )
            try? FileManager.default.removeItem(at: patchURL)
