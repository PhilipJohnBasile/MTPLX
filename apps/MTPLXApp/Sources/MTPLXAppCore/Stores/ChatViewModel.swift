...
public final class ChatViewModel: ObservableObject {
    ...
    private func cancelTurn(_ stream: ChatTurnStream, persistPartial: Bool = true) async {
        ...
        saveContext()
        await finishWorkspaceRun(stream.workspaceRunID, status: "cancelled")
        if persistPartial {
    ...
    private func startStream(
        ...
        stream.task = Task { [weak self] in
            ...
            stream.workspaceRunID = runID
            guard !Task.isCancelled, self.isRegistered(stream) else {
                await self.finishWorkspaceRun(runID, status: "cancelled")
                return
            }
    ...
    private func runToolLoop(
        ...
        loop: while !Task.isCancelled {
            ...
            if let streamError {
                await finishWorkspaceRun(runID, status: "failed", error: streamError.localizedDescription)
            ...
            if let serverMessage = stream.roundServerError {
                await finishWorkspaceRun(runID, status: "failed", error: serverMessage)
            ...
            guard let finishReason = stream.roundFinishReason else {
                await finishWorkspaceRun(runID, status: "failed", error: "Stream ended without a finish frame")
                ...
                for call in accumulatedToolCalls.values {
                    ...
                    if codingWorkspaceTool {
                        ...
                        if !approved {
                            ...
                            failureDetail = outcome.failure.map(Self.failureDetail)
                        }
                    }
                    let traceStatus: ToolTraceStatus = succeeded ? .success : .failed
                    updatePendingTrace(of: stream, id: traceId) { trace in
                        trace.status = traceStatus
                        trace.detail = failureDetail ?? Self.shortResultDetail(for: call.name, json: result)
                    }
                    if !codingWorkspaceTool {
