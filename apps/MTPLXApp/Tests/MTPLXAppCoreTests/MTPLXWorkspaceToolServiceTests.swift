...
final class MTPLXWorkspaceToolServiceTests: XCTestCase {
    ...
    func testReadOnlyGitToolsIgnoreRepositoryHooksAndDiffDrivers() async throws {
        let fixture = try makeWorkspace()
        let root = fixture.appendingPathComponent("repo", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let marker = fixture.appendingPathComponent("outside-workspace-marker")
        let hook = fixture.appendingPathComponent("hook.sh")
        try Data("#!/bin/sh\nprintf invoked > '\(marker.path)'\nprintf 'token\\0'\n".utf8)
            .write(to: hook)
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: hook.path)
        _ = try run(["git", "init", "-q"], in: root)
        let readme = root.appendingPathComponent("README.md")
        try Data("original\n".utf8).write(to: readme)
        _ = try run(["git", "add", "README.md"], in: root)
        _ = try run(["git", "-c", "core.hooksPath=/dev/null", "-c", "user.name=Test", "-c", "user.email=test@example.invalid", "commit", "-qm", "fixture"], in: root)
        ...
        for name in ["inspect_repo", "git_status", "git_diff"] {
            let response = try await object(service.dispatch(
                name: name, argumentsJSON: "{}", workspaceRoot: root.path, approved: false
            ))
            XCTAssertFalse(FileManager.default.fileExists(atPath: marker.path), name)
            if name == "git_status" {
                XCTAssertEqual(response["exit_code"] as? Int, 0)
                XCTAssertTrue((response["stdout"] as? String)?.contains("README.md") == true)
            }
            if name == "git_diff" {
                let scopes = try XCTUnwrap(response["scopes"] as? [[String: Any]])
                XCTAssertEqual(scopes.first?["exit_code"] as? Int, 0)
                XCTAssertTrue((scopes.first?["diff"] as? String)?.contains("+changed") == true)
            }
        }
    }
