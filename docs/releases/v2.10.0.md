...
| | 2.9.2 | 2.10.0 |
|---|---:|---:|
| 27B decode, 147k context | 12.0 tok/s | 18.4 tok/s |
| 27B decode, 16k / 88k context | baseline | +31% / +29% |
| 27B prefill, 88k context | baseline | +41% |
| Decode with q8 KV quantization | crash, or -50% | -4% |
| Decode with q4 KV quantization | crash, or -50% | -19% |
| Rewriting a file the model just wrote | 73.8 tok/s | 87.6 tok/s |
| Allocator growth inside one long answer | 8.6 GB | 0.6 GB |
| Multi-file agent task, wall clock | 150.2 s | 44.2 s |
| Agent first token, mid-session | full re-prefill | 0.11 s |
| 48 GB Mac, Speed model, 42k context | swap-bound | 33 tok/s |
| Qwen 3.8 Flash-Next | not supported | 61 to 76 tok/s |
...
- `MTPLX_MEMORY_BUDGET` (or `--memory-budget 48G`) reproduces any seat exactly, and the 48 GB seat is pinned by integration tests.
...
A clean same-wheel A/B measured mlx 0.32.0 against 0.32.2 at +29 percent decode and +41 percent prefill at 88k context, and +31 percent at 16k. The dependency floor moves so existing installs converge on the faster stack, and pin-guard tests fail loudly if an environment or a shipped profile ever drifts below it.
