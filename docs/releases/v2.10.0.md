...
| | 2.9.2 | 2.10.0 |
|---|---:|---:|
| 27B decode, 16k context | 43.5 tok/s | 46.6 tok/s (+7%) |
...
A clean same-wheel A/B measured mlx 0.32.0 against 0.32.2 at 23.6 to 30.4 tok/s decode at 88k (+29%), 379 to 535 tok/s prefill (+41%), and +31 percent decode at 16k in the same bench lane. Short context under the turbo profile was already compiled-kernel fast (43.9 tok/s at 21.6k, 50.3 at 6k on the new stack), so the floor bump matters most for long context and for installs still sitting on 0.32.0. The dependency floor moves so existing installs converge on the faster stack, and pin-guard tests fail loudly if an environment or a shipped profile ever drifts below it.
