...
Codex `reasoning.effort: "xhigh"` is accepted and explicitly downgraded to the
local runtime's `high` level. The requested/effective values and downgrade flag
are retained in request metadata for observability; the Responses payload still
echoes the client's requested reasoning configuration.
