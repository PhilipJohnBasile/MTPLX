...
class HookResult:
    ...
    def reject(cls, reason: str, *, status_code: int = 403) -> "HookResult":
        return cls(action=PolicyAction.REJECT, reason=str(reason), status_code=int(status_code))
...
class PolicyBus:
    ...
    def register(
        ...
        *,
        ...
        timeout = self.config.default_timeout_s if timeout_s is None else float(timeout_s)
        if timeout <= 0:
        ...
        with self._lock:
            if normalized not in self._hooks and len(self._hooks) >= self.config.maximum_hooks:
