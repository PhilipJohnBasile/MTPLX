...
class MemoryGovernorConfig:
    ...
    high_prefill_chunk_tokens: int = 1024
    critical_prefill_chunk_tokens: int = 512
    normal_prefill_chunk_tokens: int | None = None
    high_max_concurrency: int = 1
    critical_max_concurrency: int = 1
...
class MemoryGovernorDecision:
    ...
    speculative_allowed: bool | None = None
...
class RuntimeMemoryGovernor:
    ...
    def observe(self, sample: MemorySample) -> MemoryGovernorDecision:
        ...
        prefill_chunk: int | None = self.config.normal_prefill_chunk_tokens
        max_concurrency: int | None = None
        speculative_allowed: bool | None = None
        ...
        if pressure == MemoryPressureLevel.CRITICAL:
            ...
            prefill_chunk = self.config.critical_prefill_chunk_tokens
            max_concurrency = self.config.critical_max_concurrency
            ...
            prefill_chunk = self.config.high_prefill_chunk_tokens
            max_concurrency = self.config.high_max_concurrency
            speculative_allowed = None
    ...
    def apply(
        ...
        *,
        bank: Any,
        policy_callbacks: Mapping[str, Callable[[Any], None]] | None = None,
        ...
        callbacks = dict(policy_callbacks or {})
        values = {
            "prefill_chunk_tokens": decision.prefill_chunk_tokens,
            "max_concurrency": decision.max_concurrency,
            "speculative_allowed": decision.speculative_allowed,
        }
        for name, value in values.items():
            callback = callbacks.get(name)
            if callable(callback) and value is not None:
                callback(value)
