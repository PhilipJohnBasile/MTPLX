...
class SessionBankBudgetConsumer:
    ...
    def apply_budget_bytes(self, value: int, *, reason: str) -> int:
        ...
        per_session = int(getattr(self.bank, "per_session_max_bytes", requested) or requested)
...
class AttributeBudgetConsumer:
    ...
    def __init__(self, target: Any, *, name: str, attribute: str = "budget_bytes") -> None:
        self.target = target
        self.name = name
...
class UnifiedMemoryCoordinator:
    ...
    def plan(
        ...
        *,
        ...
        reason = "ready" if eligible else ("disabled" if not self.config.enabled else "unsafe_point")
