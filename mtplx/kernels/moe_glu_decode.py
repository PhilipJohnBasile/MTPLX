...
scales/biases [E, 2N, K/gs]) and the stock down pack. bits=4, group_size=32
only — anything else falls back to the stock path. Built in-house on the
affine dequant contract (w = scale*q + bias, little-endian nibbles), with
mlx's quantized.h as the reference for the packing order (Apache-2.0,
upstream dependency)."""
...
# K, groups strided lane -> lane+32. GS=32, BITS=4 hardcoded.
_SRC_A = """
    constexpr int K = 2560;
    constexpr int NGROUPS = K / 32;          // 80 groups per row
    constexpr int WPG = 4;                    // uint32 words per group (32 nibbles)
...
# One simdgroup per output dim d: for each expert, dot down_e[d, :640] with
# h[e], scale by router weight, accumulate. Iteration g covers group g; each
# lane one value inside the group (values k = g*32 + lane).
_SRC_B = """
    constexpr int NG = 20;                    // 640 / 32 groups per down row
    ...
    for (int e_slot = 0; e_slot < topk; ++e_slot) {
        ...
        const device T* srow = ds + ((size_t)e * dmodel + d) * NG;
        const device T* brow = db + ((size_t)e * dmodel + d) * NG;
        ...
        for (int g = 0; g < NG; ++g) {
            ...
            const float hv = (float)he[k];
            dot = metal::fma((float)srow[g] * q + (float)brow[g], hv, dot);
        }
        dot = simd_sum(dot);
...
def moe_glu_decode(x, gu_w, gu_s, gu_b, dn_w, dn_s, dn_b, experts, route_w):
    ...
    (h,) = ka(
        ...
        template=[("T", x.dtype), ("n_inter_c", n_inter), ("topk_c", topk)],
        grid=(tgs * 1024, 1, 1),
    ...
    (y,) = kb(
        ...
        template=[("T", x.dtype), ("dmodel_c", dmodel), ("n_inter_c", n_inter), ("topk_c", topk)],
        grid=(tgs_b * 1024, 1, 1),
        threadgroup=(1024, 1, 1),
