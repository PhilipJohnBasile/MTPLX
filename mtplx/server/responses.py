...
def _reasoning_to_chat(reasoning: Any) -> tuple[str | None, str | None, bool]:
    ...
    # MTPLX's local runtime has no level above high.  Codex is allowed to send
    # xhigh; make the downgrade explicit in request metadata/observability.
    effective = "high" if requested == "xhigh" else requested
...
def translate_request(request: ResponsesRequest) -> ResponsesConversion:
    ...
    reasoning_effort, reasoning_requested, reasoning_downgraded = _reasoning_to_chat(
        request.reasoning
    )
    ...
    if reasoning_requested is not None:
        ...
        metadata["responses_reasoning_effort_effective"] = reasoning_effort
        metadata["responses_reasoning_effort_downgraded"] = reasoning_downgraded
