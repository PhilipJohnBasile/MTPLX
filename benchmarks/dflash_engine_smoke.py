#!/usr/bin/env python3
"""Minimal same-runtime exactness smoke for the env-gated DFlash source."""
...
def main() -> int:
    ...
    try:
        ...
        source.close()
    exact = (
        list(dflash.tokens) == list(ar.tokens)
        and list(mtp_d1.tokens) == list(ar.tokens)
    )
    payload = {
        "schema": 1,
        ...
        "seed": args.seed,
        "block_size": args.block_size,
        "generation_mode": "dflash-one-hot",
        "nax_enabled": False,
        "exact_token_stream": exact,
        "ar": {
