...
class ExplodingArray:
    def tolist(self):
        raise AssertionError("disabled instrumentation must not materialize router output")
...
def test_sampling_reduces_materialization_frequency():
    tracker = ExpertLocalityTracker(max_events=10, sample_every=3, cache_capacities=(2,))
    accepted = [
        tracker.record([[index]], layer_id=1, lane="decode", num_experts=32)
...
class _FakeModel:
    ...
    def named_modules(self):
        return [("", self), ("layers.0", self.block), ("layers.0.switch_mlp", self.block.switch_mlp)]
