...
def test_run_requires_content_or_trusted_resolver(tmp_path):
    write_capture(tmp_path, "redacted", {"request_id": "x", "prompt_tokens": 2})
    ...
    with pytest.raises(ReplayPlanError):
        orchestrator.run(
            ...
            evaluators={"ok": lambda _c, _o, _b: Evaluation("ok", 1, True)},
        )

    receipt = orchestrator.run(
        plan,
        resolver=lambda envelope: {"restored": envelope.capture_id},
        candidate=lambda request: request,
        evaluators={"ok": lambda _c, _o, _b: Evaluation("ok", 1, True)},
    )
    assert receipt.promotion_applied is False
    assert receipt.decision["promote"] is True
    assert receipt.stale_check_passed is True


def test_stale_capture_is_rejected_before_candidate_execution(tmp_path):
    ...
    plan = orchestrator.build_plan()
    capture.write_text(
        json.dumps({"request_id": "case", "request": {"payload": {"prompt": "changed"}}}),
        encoding="utf-8",
    )
    called = []
    with pytest.raises(StaleReplayPlanError):
        orchestrator.run(
            plan,
            candidate=lambda request: called.append(request),
            evaluators={"ok": lambda _c, _o, _b: Evaluation("ok", 1, True)},
        )
    assert called == []
...
def test_receipt_is_atomic_private_by_default_and_listable(tmp_path):
    ...
    receipt = orchestrator.run(
        plan,
        candidate=lambda _request: {"answer": "candidate"},
        evaluators={
            "score": lambda _case, _output, _baseline: Evaluation(
                "score", score=0.8, passed=True, baseline_score=0.8
            )
        },
