...
def test_read_only_git_tools_do_not_execute_repository_programs(tmp_path, tool, driver):
    ...
    assert workspace.tool_policy["read"] == "allow"
    assert workspace.tool_policy["terminal"] == "ask"
    response = WorkspaceToolService(store).execute(workspace.id, tool, {})
    assert response["ok"] is True, response
    assert not marker.exists()
    if tool == "git_status":
        assert "README.md" in response["result"]["stdout"]
    if tool == "git_diff":
        assert "+# Changed" in response["result"]["scopes"][0]["stdout"]


@pytest.mark.parametrize("driver", ["textconv", "command"])
@pytest.mark.parametrize("scope", ["unstaged", "staged", "both"])
def test_git_diff_does_not_execute_repository_diff_drivers(tmp_path, driver, scope):
    project = _repository(tmp_path)
    marker = tmp_path / "diff-driver-marker"
    hook = tmp_path / "diff-driver.sh"
    hook.write_text(
        "#!/bin/sh\nprintf invoked > " + shlex.quote(str(marker)) + "\n",
        encoding="utf-8",
    )
    hook.chmod(0o700)
    _git(project, "config", f"diff.untrusted.{driver}", str(hook))
    (project / ".gitattributes").write_text("README.md diff=untrusted\n", encoding="utf-8")
    (project / "README.md").write_text("# Changed\n", encoding="utf-8")
    if scope in {"staged", "both"}:
        _git(project, "add", "README.md")
    store = WorkspaceStore(tmp_path / "state")
    workspace = store.create_workspace("MTPLX", str(project))
    response = WorkspaceToolService(store).execute(workspace.id, "git_diff", {"scope": scope})
    assert response["ok"] is True, response
    assert not marker.exists()
    assert any("+# Changed" in section["stdout"] for section in response["result"]["scopes"])
