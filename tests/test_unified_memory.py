...
def sample(*, total=100 * GIB, process=50 * GIB, session=5 * GIB, expert=5 * GIB, kv=5 * GIB):
...
def test_hysteresis_avoids_small_budget_churn():
    ...
    assert all(not item.applied and item.reason == "hysteresis" for item in receipt.mutations)
