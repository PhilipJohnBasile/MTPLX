    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-12 xl:col-span-4">
        <Card
          ...
          subtitle="Explicit gate state; this panel does not run a live probe"
        >
          <div className="space-y-3">
            ...
            >
              Deep doctor and require-MTP inspection completed successfully.
            </StatusBlock>
            <StatusBlock
              ...
              title="AR benchmark"
            >
              The <code>deepseek_v4</code> loader expected the mxfp4 fork layout and
              rejected all 2,320 calibrated-affine keys. The benchmark did not run;
              this is a loader/layout blocker, not a performance failure.
            </StatusBlock>
            <StatusBlock
              ...
              status="DEFERRED"
              tone="deferred"
              title="Three validated model artifacts"
            >
              <p>
                Functionally validated but memory-infeasible on this 128 GB machine.
                Registry posting is optional and was not requested.
              </p>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {[
                  "Tailfix-Calibrated-Base",
                  "Tailfix-Lean-S5",
                  "Tailfix-DSpark-Companion",
                ].map((label) => (
                  <code
                    key={label}
                    className="rounded border border-[var(--border-soft)] bg-[var(--bg-card)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]"
                  >
                    {label}
                  </code>
                ))}
...
type StatusTone = "pass" | "blocked" | "deferred";
...
function StatusBlock({
  ...
  const toneClass =
    tone === "pass"
      ? "border-[var(--accent-cool)]/30 bg-[var(--accent-cool)]/5 text-[var(--accent-cool)]"
      : tone === "blocked"
        ? "border-[var(--accent-warm)]/30 bg-[var(--accent-warm)]/5 text-[var(--accent-warm)]"
  ...
  return (
    <div className={`rounded-xl border p-3 ${toneClass}`}>
      ...
      <div className="mt-1 text-xs leading-relaxed text-[var(--text-muted)]">{children}</div>
    </div>
  );
}
